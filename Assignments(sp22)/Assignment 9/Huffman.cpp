...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if(map.size() <= 1) error("the string must contain at least two different characters");
    ...
    for (char key : map) {
        ...
        node->ch = key;
        pq.enqueue(node, map.get(key));
    }

    //construct encoding tree.
    while (pq.size() != 1) {
        EncodingTreeNode* new_node = new EncodingTreeNode;
        double zero_priority = pq.peekPriority();
        new_node->zero = pq.dequeue();
        double one_priority = pq.peekPriority();
        new_node->one = pq.dequeue();
        pq.enqueue(new_node, zero_priority + one_priority);
    }
...
string decodeText(Queue<Bit>& bits, EncodingTreeNode* tree) {
    ...
    while (!bits.isEmpty()) {
        ...
        if(tmp->zero == nullptr && tmp->one == nullptr) {
            result += tmp->ch;
            tmp = tree;
        }
    }
...
void encodeTextRec(EncodingTreeNode* tree, Map<char, Vector<Bit>>& table, Vector<Bit>& tmp) {
    if(tree->zero == nullptr && tree->one == nullptr) {
        table[tree->ch] = tmp;
    }
    if(tree->zero != nullptr) {
        tmp.add(0);
        encodeTextRec(tree->zero, table, tmp);
        tmp.remove(tmp.size() - 1);
    }
    if(tree->one != nullptr) {
        tmp.add(1);
        encodeTextRec(tree->one, table, tmp);
        tmp.remove(tmp.size() - 1);
    }
}
...
void encodeTree(EncodingTreeNode* tree, Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    bits.enqueue(1);
    encodeTree(tmp->zero, bits, leaves);
    encodeTree(tmp->one, bits, leaves);
}
