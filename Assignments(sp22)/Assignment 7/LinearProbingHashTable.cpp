...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<string> hashFn) {
    hashFunction = hashFn;
    _Capacity_Size = hashFn.numSlots(); //capacity is depended on the hash function.
    _Logical_Size = 0;
    elems = new Slot[_Capacity_Size];

    for(int i = 0; i < _Capacity_Size; i++) {
...
//worst case: O(n), n is the capacity of the table.
//best case: O(1)
bool LinearProbingHashTable::insert(const string& elem) {
    //table is full.
    if(_Logical_Size == _Capacity_Size) {
        return false;
    }
    ...
    if(elems[hashcode].type == SlotType::EMPTY ||
    ...
    if(elems[hashcode].type == SlotType::FILLED) {
        while(elems[hashcode].type == SlotType::FILLED) {
            hashcode += 1;
            if(hashcode == _Capacity_Size) {
                hashcode = 0;
            }
        }
        elems[hashcode].value = elem;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    int hashcode = hashFunction(elem);
    bool isHere = elems[hashcode].type == SlotType::FILLED && elems[hashcode].value == elem;
    ...
    while(!isHere && times < _Capacity_Size) {
        hashcode += 1;
        if(hashcode == _Capacity_Size) {
            hashcode = 0;
        }
        if(elems[hashcode].type == SlotType::EMPTY) return false;//if you don't add this line,
                                                         //the best case will be O(n).

        times += 1;
        isHere = elems[hashcode].type == SlotType::FILLED && elems[hashcode].value == elem;
        if(isHere) {
            return true;
        }
    }
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    while(elems[hashcode].type != SlotType::EMPTY && elems[hashcode].value != elem) {
        hashcode += 1;
        if(hashcode == _Capacity_Size) {
            hashcode = 0;
        }
...
void LinearProbingHashTable::printDebugInfo() const {
    // cout<<"the contents inside LinearProbingHashTable:" <<endl;
    // for(int i = 0; i < this->_Capacity_Size; i++) {
    //     cout<<"number "i + ": " << "value: " + this->elems[i].value << " type:"this->elems[i].type <<endl;
    // }
    return;
}
