...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    this->hashFn = hashFn;
    _Capacity_Size = this->hashFn.numSlots();
    _Logical_Size = 0;
...
bool RobinHoodHashTable::insert(const string& elem) {
    //table is full.
    if(_Logical_Size == _Capacity_Size) return false;

    //make sure no duplicates.
    if(contains(elem)) return false;

    int hashcode = this->hashFn(elem);

    Slot cur = {elem, 0};

    while(elems[hashcode].distance != EMPTY_SLOT) {
        if(elems[hashcode].distance < cur.distance) {
            Slot tmp = elems[hashcode];
            elems[hashcode] = cur;
            cur = tmp;
        }
        hashcode += 1;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    while(elems[hashcode].distance != EMPTY_SLOT) {
        if(elems[hashcode].distance < elem_distance) return false;
        if(elems[hashcode].value == elem) return true;
        hashcode += 1;
        if(hashcode == _Capacity_Size) {
            hashcode = 0;
        }
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    while(elems[hashcode].distance != EMPTY_SLOT) {
        if(elems[hashcode].distance < elem_distance) return false;
        if(elems[hashcode].value == elem) break;
        hashcode += 1;
        if(hashcode == _Capacity_Size) {
            hashcode = 0;
        }
    ...
    if(elems[hashcode].distance == EMPTY_SLOT) return false;
    ...
    while(elems[hashcode].distance > 0) {
        int front = (hashcode > 0) ? hashcode - 1 : _Capacity_Size - 1;
        swap(elems[front], elems[hashcode]);
        elems[front].distance -= 1;
        hashcode += 1;
        if(hashcode == _Capacity_Size) {
            hashcode = 0;
        }
