...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    if(populations.isEmpty()) error("the population is empty!");
    if(numSeats <= 0) error("numSeats must be positive!");
    if(populations.size() > numSeats) error("numsSeats are less than state");

    //construct the priority queue and result(make sure every state get one seat at beginning).
    HeapPQueue* hpq = new HeapPQueue();
    Map<string, int> result;

    for(string s : populations) {
        hpq->enqueue(DataPoint{s, static_cast<double>(-populations.get(s)) / pow(2, 0.5)});
        result.put(s, 1);
    }
    numSeats -= populations.size();

    while(numSeats > 0) {
        DataPoint tmp = hpq->dequeue();

        result.put(tmp.name, result.get(tmp.name) + 1);
        numSeats -= 1;

        int seats = result.get(tmp.name);
        tmp.weight = -populations.get(tmp.name) / sqrt(seats * (seats + 1));
        hpq->enqueue(tmp);
    }
