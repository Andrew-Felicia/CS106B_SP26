...
HeapPQueue::HeapPQueue() {
    ...
    elems = new DataPoint[allocatedSize]; //the first one is unused.
...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    if(logicalSize == allocatedSize - 1) {
        ...
        for(int i = 1; i < allocatedSize / 2; i++) {
            new_elems[i] = elems[i];
        }
    ...
    int current_index = logicalSize;

    while(parent != 0 && data.weight < elems[parent].weight) {
        swap(elems[current_index], elems[parent]);
...
DataPoint HeapPQueue::peek() const {
    if(isEmpty()) error("the hrap is empty!");
    return elems[1];
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    DataPoint result = elems[logicalSize];
    logicalSize -= 1;

    int toBeBubbleDownIndex = 1;
    int left_child = toBeBubbleDownIndex * 2;
    int right_child = toBeBubbleDownIndex * 2 + 1;

    while(!isBubbleDownDone(toBeBubbleDownIndex)) {
        if(left_child <= logicalSize && right_child > logicalSize) {//1 child.
            swap(elems[toBeBubbleDownIndex], elems[left_child]);
            toBeBubbleDownIndex = left_child;
        } else if(left_child <= logicalSize && right_child <= logicalSize) { //2 child.
            int toBeSwappedChild = 0;
            if(elems[left_child].weight <= elems[right_child].weight) {
                toBeSwappedChild = left_child;
            } else {
                toBeSwappedChild = right_child;
            }
            swap(elems[toBeBubbleDownIndex], elems[toBeSwappedChild]);
            toBeBubbleDownIndex = toBeSwappedChild;
        }
        left_child = toBeBubbleDownIndex * 2;
        right_child = toBeBubbleDownIndex * 2 + 1;
    }
    return result;
}

//helper function of dequeue, decide if the bubble down process is done.
bool HeapPQueue::isBubbleDownDone(int toBeBubbleDownIndex) {
    int left_child = toBeBubbleDownIndex * 2;
    int right_child = toBeBubbleDownIndex * 2 + 1;

    if(left_child > logicalSize && right_child > logicalSize) {  //0 child
        return true;
    } else if(left_child <= logicalSize && right_child > logicalSize) {//1 child
        if(elems[toBeBubbleDownIndex].weight <= elems[left_child].weight) {
            return true;
        }
        return false;
    } else {//2 child
        if(elems[toBeBubbleDownIndex].weight <= elems[left_child].weight &&
            elems[toBeBubbleDownIndex].weight <= elems[right_child].weight) {
            return true;
        }
        return false;
    }
