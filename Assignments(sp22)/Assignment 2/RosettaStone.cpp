...
Map<string, double> normalize(const Map<string, double>& input) {
    //check that the input map is not empty.
    if(input.isEmpty()) {
        error("the input map is empty.");
    }

    //check there's at least one non zero elements inside input map.
    int nonZeroCount = 0;
    ...
    if(nonZeroCount == 0) error("all the words' frequency are zero.");
    ...
    double squareRootOfSquareSum = sqrt(squareSum);
    for(string key : input) {
        result.put(key, input[key] / squareRootOfSquareSum);
    }
