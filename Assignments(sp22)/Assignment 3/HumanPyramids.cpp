...
/*
 * this function is very important. review it often.
 */

double weightOnBackOfRec(int row, int col, int pyramidHeight, Map<GridLocation, double>& memo) {
    //base case 1.
    if(row < 0 || row > pyramidHeight - 1) error("Invalid row and col!");
    if(col < 0 || col > row) return 0;

    //base case 2/
    if(row == 0 && col == 0) return 0;

    GridLocation curKey = {row, col};
    if(memo.containsKey(curKey)) return memo.get(curKey);

    double left  = weightOnBackOfRec(row - 1, col - 1, pyramidHeight, memo);
    double right = weightOnBackOfRec(row - 1, col, pyramidHeight, memo);
    if(left == 0) {
        memo.put(curKey, (right + 160) / 2.0);
        return (right + 160) / 2.0;
    } else if(right == 0) {
        memo.put(curKey, (left + 160) / 2.0);
        return (left + 160) / 2.0;
    } else {
        memo.put(curKey, (left + 160) / 2.0 + (right + 160) / 2);
        return (left + 160) / 2.0 + (right + 160) / 2;
    }

}

double weightOnBackOf(int row, int col, int pyramidHeight) {
    Map<GridLocation, double> memo;
...
STUDENT_TEST("check person A,B,C from handout") {
...
STUDENT_TEST("check person D, F from handout") {
