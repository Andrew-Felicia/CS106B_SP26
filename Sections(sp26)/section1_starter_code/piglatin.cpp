...
string pigLatinReturn(string input){
    ...
    return input.substr(separator) + input.substr(0, separator) + "ay";
}
...
void pigLatinReference(string &input){
    ...
    input = input.substr(separator) + input.substr(0, separator) + "ay";
}

// If you wanted to combine the efficiency of pigLatinReference with the clarity of pigLatinReturn,
//I would recommend writing a function that takes in the input string by const reference, basically
